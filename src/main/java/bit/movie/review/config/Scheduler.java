...
/*      Elements movieNameDocDaum = movieListDocDaum.get(i).select(".wrap_movie .info_tit a");
      ...
      Elements movieDateDocDaum = docDaumMovie.getElementsByClass("list_movie");
      System.out.println("check4");
      String daumDate = movieDateDocDaum.get(i).select(".txt_main:nth-child(odd)").text();
      ...
      movieDaum.setDate(daumDate);
      System.out.println("check10");
      
      boolean updateCheck = true;

      for(Movie movie : movieList) {
        if(movie.getMovieName().equals(movieDaum.getMovieName()) && movie.getDate().equals(movieDaum.getDate())) {
          movie.setCodeDaum(movieDaum.getCodeDaum());
